...
with st.sidebar:
    ...
    keyword = st.text_input("검색 키워드", value="갤럭시")
    ...
    with c2:
        if st.button("🔍 실제 수집", type="primary", use_container_width=True):
            if not sources and not (include_urls and st.session_state["custom_url_text"].strip()):
                ...
                with st.spinner("VOC 수집 중..."):
                    items = collect_all(keyword, sources, max_per_source, on_progress, custom_urls=custom_urls)
