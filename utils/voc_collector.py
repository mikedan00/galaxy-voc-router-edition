...
def collect_all(
    ...
    if "samsung" in sources:
        tasks.append(("삼성 Members 커뮤니티", lambda: collect_samsung_members(keyword, max_per_source)))
    if "naver_kin" in sources:
        tasks.append(("네이버 지식인", lambda: collect_naver_kin(f"갤럭시 {keyword} 불편", max_per_source)))
    if "naver_cafe" in sources:
        tasks.append(("네이버 카페", lambda: collect_naver_cafe(f"갤럭시 {keyword} 문제", max_per_source)))
    ...
    if "clien" in sources:
        tasks.append(("클리앙", lambda: collect_clien(keyword, max_per_source)))
    if custom_urls:
